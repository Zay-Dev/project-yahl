...
export const runVerifyCli = async (jobDir: string, prompt: string): Promise<string> => {
  let lastError: unknown;

  for (let attempt = 0; attempt <= retryCount(); attempt += 1) {
    try {
      const stdout = await spawnVerifyCliOnce(jobDir, prompt);
...
const spawnVerifyCliOnce = (jobDir: string, prompt: string): Promise<string> => {
  const logPath = path.join(jobDir, 'cli-output.json');

  if (!config.apiKey) {
    const stub = JSON.stringify({
      feedback: '[verify-stub] CURSOR_API_KEY missing',
      pass: false,
      score: 0,
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0) {
        reject(new Error(`verify cli exited ${code}${stderr ? `: ${stderr.slice(0, 200)}` : ''}`));
        return;
      }
