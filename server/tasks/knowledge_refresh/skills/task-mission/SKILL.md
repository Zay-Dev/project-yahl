...
Canonical mission framing for all helper calls in the knowledge_refresh task.
...
6. After each `/nixery(research, …)` study call, **immediately** persist with `upsert-knowledge-page` — never hold study output only in session context.
