...
5. After each `/mastermind(research, …)` study call, **immediately** persist with `upsert-knowledge-page` — never hold study output only in session context. Mastermind dual-writes `studyMd` to `studies/{slug}` and metadata JSON to `raw/study_{slug}`.
