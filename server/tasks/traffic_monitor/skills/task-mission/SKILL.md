...
0. **Resolve places** before the first probe: brand/abbrev/mall/cinema ODs → `origin_resolved` / `destination_resolved` (canonical name + district). Never treat raw user strings as unique website inputs. Reject autocomplete that changes district (e.g. MCL Kowloon Tong ≠ Telford Plaza). Persist **this-run** OD under `page: source-ops-{city_slug}`, `section: PLACE`, `mode: append` (body without `##` heading) when novel vs `source_ops_md`.
