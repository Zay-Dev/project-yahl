...
0. **Resolve places** before the first probe: brand/abbrev/mall/cinema ODs → `origin_resolved` / `destination_resolved` (canonical name + district). Never treat raw user strings as unique website inputs. Reject autocomplete that changes district (e.g. MCL Kowloon Tong ≠ Telford Plaza). **Estate/building ODs must bind to that estate/building (or its car park)** — never fall back to a nearby MTR station or area-only POI unless PLACE has an explicit `verified_proxy` note approved by Knowledge Manager. Persist **this-run** OD under PLACE via `submit-knowledge-observation` with structured evidence `{ claimed_place, bound_poi, tool }` when novel vs `source_ops_md`.
...
1a. **Cold-start trust path** (`stage_goto_reason` empty): when a durable non-fallback candidate remains (placeholder URL **or** browser + howto ≥80, not Maps, not known-failed), **lock it without browser probe**. Record `tried_sources` outcome `ok` with `what_tried: 'trusted durable cache; no re-probe'`, set `probe_fetch` to `{ trusted_cache: true, source_url: traffic_source.url, routes: [] }`, **strip OD bleed from `howto_md`**, persist/upsert the lock, **ensure a reusable `## HOWTO` block exists on `source-ops-{city_slug}`** (promote get-knowledge / session tips when missing — not PLACE-only), set `locked = true`, and **finish explore** (skip research). Do not re-validate by form-fill on cold start.
1b. **Re-entry / untrusted cache**: when platform Input includes non-empty `stage_goto_reason` (monitor transferred via `/stage(explorer)`), or cache is missing/untrusted/fallback — treat as a **fresh explore budget** (`city_attempts` starts at 0): re-resolve places if needed, decline the dead cache if still listed, then **browser probe** (do not trust-skip) → research → Maps to re-lock and finish so jump-and-continue re-enters monitor with `started_at` preserved. A successful live probe still locks and finishes without further research.
...
3. Prefer city-scoped guidelines only: `~/data/{traffic_source_file}` and `~/data/{holidays_file_prefix}_{year}.md` when **usable** (see below). Do not reuse a traffic or holidays file from another city. Keep `howto_md` as **stable core** re-fetch steps only — **never embed concrete run OD / POI autocomplete strings** (use `{resolved origin}` / “pick matching autocomplete”; POI literals → PLACE ops only). Operational tricks live in nixery `source-ops-{city_slug}`, not in ever-growing howto. Prefer `traffic_source.url` with `{origin}/{destination}` placeholders; form-fill sites document the working entry URL in howto (e.g. HKeMobility `/en/route-search/driving`) while keeping a placeholder template in `url` when supported.
4. Never reset `started_at` from context. Exit the monitor loop when `now - started_at >= monitor_minutes`.
5. Notifications via `/mastermind(propose-notification, …)` using resolved `notifyChannel` / `notifyTo` from `/nixery(resolve-notification-target, to: notify_to)` (`notify_to` remains a phone, default `91234567`). Do not claim messages were sent. Draft every body from `notifyPreference` (target prefs from the resolver — user-onboarding when `notifyIsUser`, else channel summary). Empty preference → neutral professional tone, medium detail, default language, medium proactivity. **Place labels in notification/digest/report copy must use runInput `origin` / `destination` (human labels)** — never rename the trip to a bound autocomplete POI (e.g. do not title a Maple Court run as River Station). If bound POI ≠ estate identity, treat as geocode miss or annotate `bound proxy: …` — do not silently substitute.
...
Canonical page `source-ops-{city_slug}` sections (append-only across sessions — never replace the whole page):

| Section | Role |
|---------|------|
| `## HOWTO ({provider})` | Reusable UI steps for the locked provider |
| `## PLACE` | OD / autocomplete disambiguation (concrete POI literals) |
...
- **PLACE evidence (mandatory):** include `claimed_place` (runInput / resolved estate) and `bound_poi` (autocomplete chip actually clicked). If they differ in landmark class (estate vs MTR station), set `confidence: inferred` or treat the poll as geocode fail — do not submit as durable `observed` PLACE equality.
...
- **Reusable `## HOWTO` (mandatory on city lock).** Lean `sources-{city_slug}` drops `howto_md`, so city-reusable UI steps must survive on `source-ops-{city_slug}` under `section: HOWTO ({provider})`. On every city lock (trust path **and** live probe): if that section is missing, append one body covering entry URL, Driving / mode tab, form-fill From/To + autocomplete click (not ArrowDown), Search button label, SPA spinner → miss + reload, and “must re-enter form each poll”. Promote tips already present in get-knowledge extract when the wiki section lacks them. Never replace HOWTO with PLACE-only upserts.
- **`## PLACE` (this-run OD).** After `origin_resolved` / `destination_resolved` are set, append this run’s OD under `section: PLACE` (concrete POI literals) when novel vs `source_ops_md`. Do not leave PLACE dominated by stale unrelated ODs only.
- **`## Q&A` (problem → fix).** When a poll/explore hits a recoverable failure, append a short Q&A body (`**Q:** … / **A:** …`) via `section: Q&A` so later runs reuse the fix (e.g. empty result table after Search → reload entry URL and re-fill; wrong autocomplete POI → click the PLACE-named building row). Treat real browser `ok: false` / empty extract / OD mismatch as fetch misses per miss rules — do not invent ETAs or ignore failed fills.
...
- Strip OD-specific route lists and concrete POI autocomplete examples from city-level `howto_md`; keep city-reusable tips only.
