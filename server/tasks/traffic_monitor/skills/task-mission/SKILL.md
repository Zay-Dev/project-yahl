...
- **`## Q&A` (problem → fix).** When a poll/explore hits a recoverable failure, append a short Q&A body (`**Q:** … / **A:** …`) via `section: Q&A` so later runs reuse the fix (e.g. empty result table after Search → reload entry URL and re-fill; agent-mode fill `ok:false` after Search → still extract the table).
