...
2b. Before researching a candidate, `*read(source_ops_md)` (attend only — do not `const`/`set_context` a full-blob copy) and read `tried_sources`. When `instruction_active` is false, do **not** re-probe a source whose skip/fail reason still applies — pick a different provider, or record `outcome: 'skipped'` (research-returned skips still count toward the cap of 2) without thrashing the browser. When `instruction_active` is true, `source_instruction` is a this-run free-text operator override (do not require patching knowledge); apply it instead of blind known-failed auto-skip. `*research_need_for_explore(..., instruction_active, source_instruction)` must include raw `source_instruction` when active and prefer it over “avoid already failed/skipped in source_ops”. Prior ops / knowledge `budget.*` / “attempts exhausted” / `BUDGET` / `HISTORY` lines **never** skip the research loop or mean Research quota is spent.
