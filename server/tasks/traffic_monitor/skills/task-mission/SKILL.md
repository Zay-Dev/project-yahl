...
- During explore and every 20 min in monitor: draft candidate bullets → `*filter_novel_ops_notes` against `source_ops_md` → upsert only novel markdown to `source-ops-{city_slug}` (`mode: append`, prefer `value:` / `key:`; equally fine: `page: source-ops-{city_slug}` + optional `section:` + `content:`; retry `content:` once on `ok: false`). If none novel, still bump `last_source_notes_at` in monitor (no empty upsert). Prefer promoting recurring miss/FIX patterns into `## Q&A` rather than only one-off FAIL lines.
- **Reusable `## HOWTO` (mandatory on city lock).** Lean `sources-{city_slug}` drops `howto_md`, so city-reusable UI steps must survive on `source-ops-{city_slug}` under a `## HOWTO` heading for the locked provider. On every city lock (trust path **and** live probe): if that section is missing, append one covering entry URL, Driving / mode tab, form-fill From/To + autocomplete click (not ArrowDown), Search button label, SPA spinner → miss + reload, and “must re-enter form each poll”. Promote tips already present in get-knowledge extract into that block when the wiki page lacks them. PLACE notes stay under `## PLACE` — never replace HOWTO with PLACE-only upserts.
- **`## Q&A` (problem → fix).** When a poll/explore hits a recoverable failure, append a short Q&A entry (`**Q:** … / **A:** …`) so later runs reuse the fix (e.g. empty result table after Search → reload entry URL and re-fill; agent-mode fill `ok:false` after Search → still extract the table).
