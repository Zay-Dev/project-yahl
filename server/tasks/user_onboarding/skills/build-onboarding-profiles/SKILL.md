...
4. Persist with `upsert-knowledge-page` using legacy keys (`identity`, `goals`, …). Nixery dual-writes elaborated markdown to `overview`/`brief` and structured JSON to `raw/{key}` — see `/opt/skills/knowledge-wiki-style/SKILL.md`.
