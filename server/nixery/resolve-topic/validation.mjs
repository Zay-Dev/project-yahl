...
export async function validateOutput(ctx) {
  ...
  if (parsed.ok === true) {
    if (typeof parsed.canonical !== 'string' || !parsed.canonical.trim()) {
      return { ok: false, reason: 'success gate requires non-empty canonical' };
    }

    if (typeof parsed.matchedBy !== 'string' || !parsed.matchedBy.trim()) {
      return { ok: false, reason: 'success gate requires matchedBy' };
    }
