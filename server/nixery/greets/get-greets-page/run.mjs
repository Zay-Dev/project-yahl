...
const artifactName = (entity, page) => {
  const entitySeg = String(entity).replace(/[^a-zA-Z0-9._-]+/g, '-');
  const pageSeg = String(page).replace(/[^a-zA-Z0-9._-]+/g, '-');

  return `wiki-${entitySeg}-${pageSeg}.md`;
};
...
const main = async () => {
  ...
  const markdown = artifactName(entity, page);
