...
import { applyApprovedTransfers } from '/opt/nixery/plugin/lib/dist/index.js';
...
const main = async () => {
  ...
  try {
    const transferResult = await applyApprovedTransfers();
    const gate = {
      ok: true,
      approvedTransfersApplied: transferResult.approvedTransfersApplied,
      targetTopics: transferResult.targetTopics,
    };

    await fs.writeFile(outputPath, `${JSON.stringify(gate, null, 2)}\n`, 'utf8');
    logProgress(defId, `done ok=true applied=${transferResult.approvedTransfersApplied}`);
  } catch (error) {
    const gate = {
      ok: false,
