...
| Mastermind stack | Works | Gateway (port 4100) and worker (cron/approvals + optional WhatsApp Web client) in `docker compose`; stage verify via nixery `verify.defId`; `/mastermind(...)` for policies, dispatch, notifications; `/nixery(...)` for knowledge, topic resolve, tidy, QA, media-to-text, LLM helpers, and stage-verify. Stack probe via `pnpm run doctor`. |
