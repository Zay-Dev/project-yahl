...
| Cron `runInput` | Works | Cron jobs accept a string-map `runInput` validated against the task’s typed `runInput` fields (text / textarea / enum + defaults); web cron form has schedule presets (daily / hourly / every N minutes / weekday / custom). |
