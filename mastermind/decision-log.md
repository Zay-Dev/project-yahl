...
- **2026-08 cleanup:** removed `knowledge_capture`, cron shim `auto_knowledge_refresh`, mastermind `evaluate-knowledge-refresh`, nixery `run-knowledge-manager` / `tidy-knowledge` / `knowledge-qa-review`. Overnight = cron or `dispatch-task-run` → `knowledge_manager` directly.
