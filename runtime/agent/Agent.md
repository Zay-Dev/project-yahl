...
- **`mastermind`**：参数 `{ "skill": "<name>", "args": { ... } }`。用于 `/mastermind(...)`。技能：`research`、`extract-info`（workspace 文件 RAG，需 `source` + `need`）、`get-knowledge`（mastermind 读 wiki 语料，写 `~/knowledge/{key}.json`，仅返回 key/path）、`upsert-knowledge-page`（写 wiki，仅 `key`/`value`/`topic` 或 `page`/`content`）、`media-to-text`、`plan`、`design-questions`、`propose-notification`（起草 outbound 提案，不直接发送；需人工批准）。`get-knowledge` 后读 `~/knowledge/{key}.json` 的 `.extracted`。详见 `/opt/skills/mastermind/SKILL.md`。返回 `{ ok, data }` 或 `{ ok: false, error }`。
