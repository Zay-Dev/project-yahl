...
- **`mastermind`**：参数 `{ "skill": "<name>", "args": { ... } }`。用于 `/mastermind(...)`。技能：`research`、`extract-info`（workspace 文件 RAG，需 `source` + `need`）、`upsert-knowledge-page`（写 wiki，仅 `key`/`value`/`topic` 或 `page`/`content`）、`media-to-text`、`plan`、`design-questions`、`propose-notification`（起草 outbound 提案，不直接发送；需人工批准）。知识读取用 orchestrator `nixeryRun: get-knowledge`，读 `~/nixery/get-knowledge/{output}`。任务 mission 用 stage logic 的 `*load_task_mission(...)` 加载，不注入全局 system prompt。详见 `/opt/skills/mastermind/SKILL.md`。返回 `{ ok, data }` 或 `{ ok: false, error }`。
