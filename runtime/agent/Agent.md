...
- **`nixery`**：参数 `{ "defId": "<def>", "args": { ... } }`。用于 `/nixery(...)` — **`submit-knowledge-observation`**（唯一知识写入口）、`research`、`extract-info`、`design-questions`、`consult-breaking-change`、`media-to-text`、`resolve-notification-target`。经理任务另可 inline `list-pending-observations` / `apply-manager-topic`。`run-knowledge-manager` / `upsert-knowledge-page` / `dedup-knowledge` / `tidy-knowledge` / `knowledge-qa-review` 仅 Knowledge Manager allowlist。知识读取用 orchestrator `nixeryRun: get-knowledge`，读 `~/nixery/get-knowledge/{output}`。详见 `/opt/skills/nixery/SKILL.md`。`{ ok: false, error }` 时读 `error`、改 args 再调；每 stage 最多 `YAHL_NIXERY_INLINE_RETRY_MAX`（默认 3）次软失败，之后结束 stage。Def YAML `output.retry`（默认 3）控制 validation 失败后容器重跑次数（与 inline soft-fail 无关）。
