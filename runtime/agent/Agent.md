...
- **`mastermind`**：参数 `{ "skill": "<name>", "args": { ... } }`。用于 `/mastermind(...)`。技能：`resolve-topic-policy`、`media-to-text`、`propose-notification`（起草 outbound 提案，不直接发送；需人工批准）。规划用 orchestrator `nixeryRun: plan` / `plan-study`。详见 `/opt/skills/mastermind/SKILL.md`。返回 `{ ok, data }` 或 `{ ok: false, error }`。
