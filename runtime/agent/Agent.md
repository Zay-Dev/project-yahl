...
- **`mastermind`**：参数 `{ "skill": "<name>", "args": { ... } }`。用于 `/mastermind(...)`。技能：`research`、`extract-info`（workspace 文件 RAG，需 `source` + `need`）、`media-to-text`、`plan`、`design-questions`、`propose-notification`（起草 outbound 提案，不直接发送；需人工批准）。详见 `/opt/skills/mastermind/SKILL.md`。返回 `{ ok, data }` 或 `{ ok: false, error }`。
