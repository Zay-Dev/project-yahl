...
- **`mastermind`**：参数 `{ "skill": "<name>", "args": { ... } }`。用于 `/mastermind(...)`。技能：`research`、`extract-info`（workspace 文件 RAG，需 `source` + `need`）、`media-to-text`、`design-questions`、`propose-notification`（起草 outbound 提案，不直接发送；需人工批准）。规划用 orchestrator `nixeryRun: plan` / `plan-study`。详见 `/opt/skills/mastermind/SKILL.md`。返回 `{ ok, data }` 或 `{ ok: false, error }`。
- **`nixery`**：参数 `{ "defId": "upsert-knowledge-page|dedup-knowledge", "args": { ... } }`。用于 `/nixery(...)` 写 wiki 或维护 dedup。知识读取用 orchestrator `nixeryRun: get-knowledge`，读 `~/nixery/get-knowledge/{output}`。详见 `/opt/skills/nixery/SKILL.md`。
