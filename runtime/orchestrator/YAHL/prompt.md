...
Use the **`mastermind`** tool for `/mastermind(...)` platform skills (list-topic-policies, resolve-topic-policy, patch-topic-policy, evaluate-knowledge-refresh, dispatch-task-run, media-to-text, propose-notification). Read `/opt/skills/mastermind/SKILL.md`. **Do not use mastermind for verify** — stages with a `verify:` object (or `verify: true`) are scored by the orchestrator via nixery `verify.defId` after the stage finishes.
